...
x1<-as.data.table(read.csv(
  file = "C:/Users/bened/Desktop/Vrije/Lezioni/Dissertation/Dataset/Italy/Usati/FAOSTAT_cerealsdata_6-30-2021.csv"
))
...
summary(cajo)
vecm<-VECM(y_new,1,r=2,include = "none",beta=NULL,estim = "ML")
summary(vecm)
print(vecm)

#STANDARDIZE DATA
library(dplyr)
y_norm <- y_new %>% mutate_at(c("Cereals", "N2O","CH4","C02"), ~(scale(.) %>% as.vector))
#NO CONST OR TREND
...
library(dplyr)
colnames(y)=c("Cereals","N2O","CH4","C02","Temp","Preci")
y_norm <- y %>% mutate_at(c("Cereals", "N2O","CH4","C02","Temp","Preci"), ~(scale(.) %>% as.vector))
#NO CONST OR TREND
cajo<-ca.jo(y_norm,type="eigen",ecdet="none",K=2,spec="longrun",season=NULL,dumvar=NULL)
summary(cajo)
#TREND
